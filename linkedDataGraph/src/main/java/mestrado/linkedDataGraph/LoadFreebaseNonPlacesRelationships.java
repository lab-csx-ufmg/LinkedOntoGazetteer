...
public class LoadFreebaseNonPlacesRelationships {

	private static Logger LOGGER = Logger.getLogger(LoadFreebaseNonPlacesRelationships.class);
	...
	public static void main(String[] args) throws IOException {
    	...
    	BufferedReader reader = new BufferedReader(new FileReader(System.getProperty("freebase.nonPlace.fpath")));
    	...
    	while((line = reader.readLine()) != null) {
    		...
    		String nonplaceId = split[0].substring("http://rdf.freebase.com/ns/".length());
    		String placeId = split[2].substring("http://rdf.freebase.com/ns/".length());
    		
    		String predicate = split[1];
    		double ms0 = System.currentTimeMillis();
    		Place place = placeBO.retrievePlaceByFreebaseId(placeId);
    		double ms1 = System.currentTimeMillis();
    		
    		LoadFreebaseNonPlacesRelationships.LOGGER.info("@TIME@RETRIVING PLACE:\t" + (ms1-ms0) + "ms");
    		
    		if(place != null) {
    			ms0 = System.currentTimeMillis();
    			NonPlace nonplace  = nonPlaceBO.retrieveNonPlaceByFreebaseId(nonplaceId);
    			ms1 = System.currentTimeMillis();
    			LoadFreebaseNonPlacesRelationships.LOGGER.info("@TIME@RETRIVING NON-PLACE:\t" + (ms1-ms0) + "ms");
    			
    			if(nonplace != null) {
    				Place2NPlaceRel edge = placeBO.addRelatedNonPlace(place, nonplace, predicate);
    				if(edge != null) {
    					LoadFreebaseNonPlacesRelationships.LOGGER.info("EDGE "+ edge +" CREATED:\t" + nonplaceId + "\t" + placeId + "\t" + predicate);
    				} else {
    					LoadFreebaseNonPlacesRelationships.LOGGER.error("FAILED TO CREATE EDGE:\t" + nonplaceId + "\t" + placeId + "\t" + predicate);
    				}
    			} else {
    				LoadFreebaseNonPlacesRelationships.LOGGER.error("Missing NONPLACE:\t" + nonplaceId + "\t" + placeId + "\t" + predicate);
    			}
    		} else {
    			LoadFreebaseNonPlacesRelationships.LOGGER.error("Missing PLACE:\t" + nonplaceId + "\t" + placeId + "\t" + predicate);
    		}
